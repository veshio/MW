...
function App() {
  ...
  // Synchronized audio playback across all players
  useEffect(() => {
    if (!game?.playback) {
      audioPlayer.stop()
      return
    ...
    const { previewUrl, startedAt, isPlaying } = game.playback

    if (isPlaying && previewUrl) {
      const now = Date.now()
      const elapsed = now - startedAt

      // Only play if we're within the 30-second window
      if (elapsed < 30000 && elapsed >= 0) {
        audioPlayer.play(previewUrl).catch(err => {
          console.error('Audio playback failed:', err)
        })
        // Seek to correct position for sync
        if (audioPlayer.audio) {
          audioPlayer.audio.currentTime = elapsed / 1000
        }
      }
    } else {
      audioPlayer.pause()
              ...
              <div className="space-y-2">
                {loadingPlaylists ? (
                  <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 text-center">
                    ...
                    <p className="text-white/70">Loading playlists from Spotify...</p>
                  </div>
                ) : playlists.length === 0 ? (
                  <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 text-center">
                    <div className="text-4xl mb-3">❌</div>
                    <p className="text-white/70">No playlists available</p>
                    <button onClick={loadPlaylists} className="mt-3 text-amber-400 hover:text-amber-300">
                      Retry
                    </button>
                  </div>
                ) : (
